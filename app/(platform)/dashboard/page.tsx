    ...
    <div style={{ padding: '28px 32px 48px', maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 24 }} className="dash-cols">
                    ...
                    >
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--g900)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                          ...
                          {article.aiGenerated && (
                            <span style={{ marginLeft: 6, fontSize: 11 }} title="AI-Generated">🤖</span>
                          )}
                        </div>
                ...
                {data.clusters.map((cluster, i) => (
                  <div
                    ...
                    style={{
                      ...
                      gap: 12,
                      padding: '12px 0',
                      borderBottom: i < data!.clusters.length - 1 ? '1px solid var(--g100)' : 'none',
                    }}
                  >
                    <div
                      style={{
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--g900)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {cluster.title}
                      </div>
                      <div style={{ fontSize: 11, color: trendColor(cluster.trend), fontWeight: 600, marginTop: 2 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          {/* Quick Actions */}
          <Card>
            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--g900)', marginBottom: 14 }}>
              Quick Actions
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              <style>{`
                .qa-link:hover { background: var(--g50); color: var(--g900); }
              `}</style>
            </div>
          </Card>
            ...
            {data && data.teamCount > 1 ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <span style={{ fontSize: 13, color: 'var(--g600)' }}>
                  {data.teamCount} member{data.teamCount !== 1 ? 's' : ''}
                </span>
              </div>
            ) : (
...
function CardHead({ title, href, linkText }: { title: string; href: string; linkText: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
      ...
      <Link href={href} style={{ fontSize: 12, fontWeight: 600, color: 'var(--mint-d)' }}>
        {linkText} →
      </Link>
    </div>
  )
...
function StatCard({
  ...
  return (
    ...
    >
      ...
      <div>
        <div style={{ fontFamily: 'var(--mono)', fontSize: 24, fontWeight: 700, color: 'var(--g900)', lineHeight: 1.1 }}>
          {value}
        </div>
        ...
        {detail && (
          <div style={{ fontSize: 11, color: 'var(--g400)', marginTop: 4 }}>{detail}</div>
        )}
      </div>
    </div>
  )
