...
export default function EditorPage() {
  ...
  useEffect(() => {
    if (searchParams.get('smartGenerate') === 'true') {
      console.log('[Editor] smartGenerate=true detected, reading sessionStorage...')
      try {
        const raw = sessionStorage.getItem('smartArticle')
        console.log('[Editor] sessionStorage smartArticle:', raw ? `${raw.length} chars` : 'null')
        if (raw) {
          const data = JSON.parse(raw)
          console.log('[Editor] Parsed data - title:', data.title, '| tiptapContent keys:', data.tiptapContent ? Object.keys(data.tiptapContent) : 'none', '| content length:', data.content?.length || 0)
          if (data.title) setTitle(data.title)
          if (data.tiptapContent) {
            console.log('[Editor] Setting tiptapContent with', (data.tiptapContent as { content?: unknown[] })?.content?.length || 0, 'blocks')
            setContent(data.tiptapContent)
          }
          ...
          console.log('[Editor] Smart article loaded successfully, screen set to editor')
        }
      } catch (err) {
        console.error('[Editor] Failed to parse smartArticle from sessionStorage:', err)
      }
    }
  ...
  async function startGenerate() {
    ...
    console.log('[Editor] startGenerate called with prompt:', prompt)
    ...
    // Call the API
    try {
      console.log('[Editor] Calling /api/ai/generate...')
      ...
      const data = await res.json()
      console.log('[Editor] /api/ai/generate response:', res.status, '| title:', data.title, '| content length:', data.content?.length || 0)
      ...
      setScreen('editor')
      console.log('[Editor] Article generated, editor screen active')
    } catch (err) {
