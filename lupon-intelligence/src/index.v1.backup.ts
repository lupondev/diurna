...
async function syncS3ToClickHouse(env: Env): Promise<string> {
  ...
  const fileName = `${now.getUTCFullYear()}${pad(now.getUTCMonth()+1)}${pad(now.getUTCDate())}${pad(now.getUTCHours())}00.csv.gz`;

  const sql = `INSERT INTO lupon.auctions (calculation_timestamp,auction_id,publisher_id,geo,device_type,screen_resolution,event_type,bidder_code,gpt_code,cpm,is_winning,advertiser,is_floor_matched,size,currency,time_to_respond,floor_price_used) SELECT calculation_timestamp,auction_id,toUInt32(publisher_id),geo,device_type,screen_resolution,event_type,bidder_code,gpt_code,toFloat64(cpm),toUInt8(is_winning='true'),advertiser,toUInt8(is_floor_matched='true'),size,currency,toUInt32(time_to_respond),toFloat64(floor_price_used) FROM s3('https://lupon-auctions.s3.us-west-2.amazonaws.com/${fileName}','${env.AWS_ACCESS_KEY_ID}','${env.AWS_SECRET_ACCESS_KEY}','CSVWithNames')`;
...
async function saveResults(env: Env, analysis: any, chStats: Record<string, any>): Promise<void> {
  ...
  for (const u of analysis.floor_updates || []) {
    ...
    const impressions = s ? Math.round(s.total * (s.fill_pct / 100)) : 0;
    const revWithout = impressions ? Math.round(impressions * cur / 1000 * 100) / 100 : 0;
    const revWith = impressions ? Math.round(impressions * rec / 1000 * 100) / 100 : 0;
